...
if (isset($_POST["add_to_cart"])) {
    if (isset($_SESSION["shopping_cart"])) {
        ...
        if (in_array($_POST["id_discount"], $item_array_id) && in_array($_POST["size"], $item_array_size)) {
            foreach ($_SESSION["shopping_cart"] as $keys => $values) {
                if ($_SESSION['shopping_cart'][$keys]['item_id'] == $_POST["id_discount"] && $_SESSION['shopping_cart'][$keys]['item_size'] == $_POST["size"]) {
...
<body>
                                    ...
                                    <div class="card-body ">
                                        ...
                                        ?>
                                            ...
                                            <div class="row justify-content-center align-items-center align-self-center p-0 m-0 border-bottom overflow-hidden">
                                                ...
                                                <div class="col"><a class="navbar-cart-product-close" href="rest_display.php?action=clear&id=<?php echo $values["item_id"]; ?>&size=<?php echo $values["item_size"]; ?>"><i class="fa fa-times"> </i></a></div>
                                            </div>
                                        <?php
                        ...
                        <div class="card" style="width: 18rem;">
                            ...
                            <form method="post" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>">
                                <div class="d-flex justify-content-between gx-5 gy-3">
                                    ...
                                    <input type="hidden" name="hidden_price" onload="display_price(this.id);" class="noraml" id="<?= $row["id"] ?>"  data-id="<?= $row["id"]; ?>">
                                        <span class="h4 text-success result"   data-id="<?= $row["id"]; ?>"></span>                              
                                </div>
                                ...
                                <div class="d-flex flex-column bd-highlight mb-1">
                                    ...
                                    <div class="p-0">
                                        ...
                                        ?>
                                            ...
                                            ?>
                                                    <input type="radio" name="size" class="btn-check radio"  value="<?= $rows["size"] ?>" id="<?= $rows["id"] ?>" data-ids="<?= $row["id"]; ?>" autocomplete="off">
                                            ...
                                            while ($rows = mysqli_fetch_array($queryy)) {
                                            ?>
                                               <input type="hidden" name="size" id="size" value="<?= $rows["size"]; ?>">
                                              <input type="hidden" name="id_discount" id="id_discount" value="<?= $row["id"]; ?>">
                                              <input type="hidden" name="hidden_name" id="hidden_name" value="<?php echo $row["name"]; ?>" />
                            ...
                            <div class="card-footer text-muted bg-transparent border-0">
                                <div class="d-grid gap-2 col-7 mx-auto">
                                    <button class="btn btn-outline-success" type="submit" name="add_to_cart">Add To Cart <i class="fas fa-cart-plus"></i></button>
                                </div>
                            </div>
                          </form>
                        </div>
    ...
    <script>
        ...
        $('.list').click(function(){
            ...
            $(".card .card-body .radio").change(function() {
                $('.card .card-body  .radio').prop('checked', false);
                $(this).prop('checked') ? $(this).prop('checked', false) : $(this).prop('checked', true);
         
               
                var id = $('input[name="size"]:checked').attr('id');
              
                console.log(id);
                var id_card =  $('input[name="size"]:checked').attr('data-ids');
                ...
                $.ajax({
                    ...
                    success: function(data) {
                        console.log(data);
                        $('span[data-id="' + id_card +'"]').html(data + '€');
                        $('input[data-id="' + id_card +'"]').val(data);
     ...
     <script>
    //    Display price for all product into card when reload page Min Pirce
        function display_price(id) {
            // var result = $(this).parent().parent().parent().prev().prev().children("span").attr("data-id");
            ...
            $.ajax({
                ...
                success: function(data) {
                    // console.log(data);
                    $('span[data-id="' + id +'"]').html(data + '€');
                    $('input[data-id="' + id +'"]').val(data);
                }
